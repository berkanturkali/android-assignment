...
import com.arabam.android.assignment.listing.databinding.FragmentSelectModelBinding
...
class SelectModelFragment : BottomSheetDialogFragment() {
    ...
    private val containerViewModel by viewModels<CategoryContainerFragmentViewModel>(ownerProducer = { requireParentFragment().requireParentFragment() })
    ...
    private suspend fun mapModels(models: Array<CategoryItem>) = withContext(Dispatchers.Default) {
        models.map(CategoryItem::name).toTypedArray()
    }
